            ...
            for c in self.cargos:
                for p in self.planes:
                    for a in self.airports:
                        ...
                        unl = Action(expr("Unload({}, {}, {})".format(c, p, a)),
                                     [precond_pos, precond_neg],
                                     [effect_add, effect_rem])
                        unloads.append(unl)
            return unloads
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    pos = [expr('At(C1, SFO)'),
           ...
           expr('At(P1, SFO)'),
           expr('At(P2, JFK)')]
    at_dic = {'C1': ['JFK', 'ATL', 'ORD'], 'C2': ['SFO', 'ATL', 'ORD'], 'C3': ['SFO', 'JFK', 'ORD'],
               'C4': ['SFO', 'JFK', 'ATL'], 'P1': ['JFK', 'ATL', 'ORD'], 'P2': ['SFO', 'ATL', 'ORD']}
    neg = [expr('At({}, {})'.format(c, a)) for c in at_dic.keys() for a in at_dic[c]]
